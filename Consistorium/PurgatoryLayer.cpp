...
b2Vec2 PurgatoryLayer::GetLayerRange()
{
...
void PurgatoryLayer::Generate(EntityManager& entityManager, SpecialPlacesManager& placesManager)
{
	...
	for (int i = GetLayerRange().x; i >= GetLayerRange().y; i -= Globals::BLOCK_HEIGHT)
	{
		...
		for (int j = -Globals::LAYER_WIDTH_IN_BLOCKS / 2; j < Globals::LAYER_WIDTH_IN_BLOCKS / 2; j += Globals::BLOCK_WIDTH)
		{
			...
			currentPosition.x = j;
			currentPosition.y = i;
			Entities::Block* block = factory.createBlock(currentPosition, current);
	...
	for (int y = 0; y < cache.size(); y++)
	{
		...
		{
			std::shared_ptr<SpecialPlace> place = placesManager.getRandomPlace("Purgatory");
